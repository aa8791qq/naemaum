...
//			PreparedStatement ps = con.prepareStatement(query);
			...
			// 방법2 : ? 활용하기(여러개 사용 가능)
			String 	query1 = "update todo ";
					query1 += "set done = ?";
					query1 += "where todo_id = ?"; // 여러줄로 쓸때에는 공백삽입을 주의해야함(변경가능)
			PreparedStatement ps1 = con.prepareStatement(query1);
			ps1.setString(1, todoDTO.getDone()); // 첫번째 물음표에 값을 넣어달라
			ps1.setInt(2, todoDTO.getTodo_id()); // 첫번째 물음표에 값을 넣어달라
